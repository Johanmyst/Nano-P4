theory NP4_Option_State_Semantics
  imports
  Complex_Main
  NP4_Option_State_Values
...
inductive small_step :: "(statOrDecl * state * nat) \<Rightarrow> (statOrDecl * state * nat) \<Rightarrow> bool"
 (infix "\<leadsto>" 55) where
  ...
  | AssignNone: "s vName = None \<Longrightarrow> eval e s v \<Longrightarrow> (AssignmentStat (NameLVal vName) e, s, n) \<leadsto> (EmptyStat, s (vName := Some v), n-1)"
  | AssignSome: "s vName = Some v' \<Longrightarrow> eval e s v \<Longrightarrow> (AssignmentStat (NameLVal vName) e, s, n) \<leadsto> (EmptyStat, s (vName := Some v), n-1)"
  | VarDecl:    "(VariableDecl (NameLVal vName) (None), s, n) \<leadsto> (EmptyStat, s (vName := None), n-1)"
  | VarInit:    "eval e s v \<Longrightarrow> (VariableDecl (NameLVal vName) (Some e), s, n) \<leadsto> (EmptyStat, s (vName := Some v), n-1)"
  | ConstInit:  "eval e s v \<Longrightarrow> (ConstantDecl (NameLVal vName) e, s, n) \<leadsto> (EmptyStat, s (vName := Some v), n-1)"
...
next case (AssignNone e s v vName n) thus ?case using eval_deterministic by auto
next case (AssignSome e s v vName n) thus ?case using eval_deterministic by auto
...
definition stateTyping :: "typeEnv \<Rightarrow> state \<Rightarrow> bool" (infix "\<TTurnstile>" 50)
  where "\<Gamma> \<TTurnstile> s \<longleftrightarrow> (\<forall>x v. (s x) = Some v \<longrightarrow> getValType v = \<Gamma> x)"
...
proof (induct rule: small_step_induct)
     case (AssignNone e s v vName n) thus ?case using expr_preservation stateTyping_def by fastforce
next case (AssignSome e s v vName n) thus ?case using expr_preservation stateTyping_def by fastforce
